...
# coding: UTF-8
# -*- coding: UTF-8 -*-
# app/controllers/riesgo.rb

class Riesgo < Controller
    def initialize
        @riesgos = load_model("riesgos")
    end
    ...
    def guardar
        ...
        begin
            ...
            rpta = { :tipo_mensaje => "success", :mensaje => ["Se ha registrado los cambios en los riesgos", array_nuevos] }.to_json
        rescue StandardError => e #ZeroDivisionError
            rpta = { :tipo_mensaje => "error", :mensaje => ["Se ha producido un error en guardar la tabla de riesgos", e] }.to_json
        end
    ...
    def crear(temp_id, codigo, descripcion)
        id_generado = @riesgos.crear(codigo, descripcion)
        {:temporal => temp_id, :nuevo_id => id_generado}
    end

    private
    def editar(id, codigo, descripcion)
        @riesgos.editar(id, codigo, descripcion)
    end
